...
| **`source-and-full-bit-build-plan.md`** · **`source-stage0-checkpoint.md`** | 🔨 **ACTIVE — the current plan** (supersedes `capture-build-plan.md`, now in `old/`): make a *regular bit* fully itself (source · tags · rich text, loose or placed) + the inbox as the *view* of loose bits. **Stage 0 (source made first-class · bookmark retired) applied to cloud — D-102**; the sign-off + full model on paper live in the Stage-0 checkpoint. Stages 1–3 ahead (rich text + the workspace · intake · the views) |
