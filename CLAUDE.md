...
| **`lexicon.md`** | **The precise words** — the eight stored things (things · acts · vocabulary) vs the computed surfaces, parts of a bit, the acts, and RETIRED words that cause drift. Check terminology here before using a term two ways |
...
| **`capture-build-plan.md`** | 🔨 **ACTIVE — the collecting front door** (web clipper · loose bits · the inbox · call-in + source provenance): 5 slices, Cap-Checkpoints A–D. **Slice 1 (source + the inbox) applied — D-100.** Next: Slice 2 (bookmark card + inbox page). Slices 1–3 alone dissolve the S2 Apple-Notes friction |
| **`gather-build-plan.md`** | 🔨 **ACTIVE — connect a bit into a thought** (type `[[` while writing → a chip; "gathered into" backlinks): 3 stages G1–G3, reviewed + revised 2026-07-25; the graph picture stays parked. **G1 (the `reference` table) applied — D-101.** Next: G2 (the `[[` picker + chip) |
...
- **Terminology is governed by `lexicon.md`** — the single source for the project's words (the eight stored things vs computed surfaces; the retired words that cause drift). Consult it before naming or defining anything; when the model changes, update the lexicon in the same pass. It becomes the **naming authority for code** (table/type/field names) at the translation step. Never duplicate its definitions elsewhere — point to it.
...
**bit** + **board** (D-034); a board's spatial rendering is "canvas." The owner's word *fragment* = bit. The tap-a-tag view is **the pull** (ruled, D-070). The saved-URL bit type is **bookmark** (D-074 — "link" has zero live meanings). No open names.
