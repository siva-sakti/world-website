#!/bin/bash
# ============================================================================
# run-link-native.sh — the link bit-type proof, on native Postgres 17.
...
#     where bookmark still exists; that is why the link refusals live in their own
#     file, run against the full chain below, where 'link' is a real allowed type.)
#
#   PROOF 2 (the link migration): apply EVERY migration THROUGH
#     20260901000001_link_type.sql and run link-attacks.sql — the link refusals
#     (proven against the substance branch, not type_allowed) + the filename search
#     (inherited from the audio migration, no link-specific index work).
#
...
roles "$DB_INIT"
if psql -d "$DB_INIT" -v ON_ERROR_STOP=1 -f "$W/supabase/migrations/20260721000001_init.sql" >/tmp/link_init_apply.txt 2>&1; then
  echo "init migration applied clean OK"
else
  echo ">>> INIT APPLY FAILED:"; tail -5 /tmp/link_init_apply.txt; exit 1
fi
...
# ============================================================================
echo ""
echo "=== PROOF 2: the link migration (full chain + link-attacks.sql) ==="
...
for m in $(ls "$W"/supabase/migrations/*.sql | sort); do
  if ! psql -d "$DB_ALL" -q -v ON_ERROR_STOP=1 -f "$m" >/dev/null 2>/tmp/link_all_apply.txt; then
    echo ">>> APPLY FAIL $(basename "$m")"; tail -5 /tmp/link_all_apply.txt; exit 1
  fi
done
echo "all $(ls "$W"/supabase/migrations/*.sql | wc -l | tr -d ' ') migrations applied clean OK (incl. 20260901000001_link_type.sql)"
...
psql -d "$DB_ALL" -v ON_ERROR_STOP=1 -c "grant usage on schema public to anon, authenticated;
  grant select on all tables in schema public to anon;
  grant all on all tables in schema public to authenticated;" >/dev/null
psql -d "$DB_ALL" -f "$W/verification/link-attacks.sql" 2>&1
A2=${PIPESTATUS[0]}
[ "$A2" = "0" ] && echo "PROOF 2 PASSED OK — the link migration is proven" \
  || { echo ">>> PROOF 2 FAILED (exit $A2)"; exit 1; }

dropdb --if-exists "$DB_INIT" 2>/dev/null; dropdb --if-exists "$DB_ALL" 2>/dev/null
echo ""
echo "=== link proof COMPLETE — existing suite green, link migration proven ==="
