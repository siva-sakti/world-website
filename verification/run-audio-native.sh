#!/bin/bash
# ============================================================================
# run-audio-native.sh — the audio bit-type proof, on native Postgres 17.
...
#     where bookmark still exists; that is why the audio refusals live in their own
#     file, run against the full chain below, where 'audio' is a real allowed type.)
#
#   PROOF 2 (the audio migration): apply EVERY migration THROUGH
#     20260830000003_audio_type.sql and run audio-attacks.sql — the audio refusals
#     (proven against the substance branch, not type_allowed) + the filename search.
#
# Re-run after any change touching the bit type set, the substance rule, or the
# search_tsv/file_name index. Expected: both proofs exit 0.
# ============================================================================
set -uo pipefail
export PATH="/opt/homebrew/opt/postgresql@17/bin:$PATH"
W=$(cd "$(dirname "$0")/.." && pwd)
DB_INIT=proof_audio_init
DB_ALL=proof_audio
...
roles "$DB_INIT"
if psql -d "$DB_INIT" -v ON_ERROR_STOP=1 -f "$W/supabase/migrations/20260721000001_init.sql" >/tmp/audio_init_apply.txt 2>&1; then
  echo "init migration applied clean OK"
else
  echo ">>> INIT APPLY FAILED:"; tail -5 /tmp/audio_init_apply.txt; exit 1
fi
...
# ============================================================================
echo ""
echo "=== PROOF 2: the audio migration (full chain + audio-attacks.sql) ==="
...
for m in $(ls "$W"/supabase/migrations/*.sql | sort); do
  if ! psql -d "$DB_ALL" -q -v ON_ERROR_STOP=1 -f "$m" >/dev/null 2>/tmp/audio_all_apply.txt; then
    echo ">>> APPLY FAIL $(basename "$m")"; tail -5 /tmp/audio_all_apply.txt; exit 1
  fi
done
echo "all $(ls "$W"/supabase/migrations/*.sql | wc -l | tr -d ' ') migrations applied clean OK (incl. 20260830000003_audio_type.sql)"
...
psql -d "$DB_ALL" -v ON_ERROR_STOP=1 -c "grant usage on schema public to anon, authenticated;
  grant select on all tables in schema public to anon;
  grant all on all tables in schema public to authenticated;" >/dev/null
psql -d "$DB_ALL" -f "$W/verification/audio-attacks.sql" 2>&1
A2=${PIPESTATUS[0]}
[ "$A2" = "0" ] && echo "PROOF 2 PASSED OK — the audio migration is proven" \
  || { echo ">>> PROOF 2 FAILED (exit $A2)"; exit 1; }

dropdb --if-exists "$DB_INIT" 2>/dev/null; dropdb --if-exists "$DB_ALL" 2>/dev/null
echo ""
echo "=== audio proof COMPLETE — existing suite green, audio migration proven ==="
