...
| F note | **X1 date format RULED (owner, 2026-09-06, other window):** “Sep 6, 2026” / “Sep 6, 2026, 9:41 AM” — year ALWAYS, no relative-only. `lib/dates.ts` is now nonconforming by design of its era (fmt elides this year's year; ago is relative-only) — the SWEEP rewrites it post-Phase-F; NEW date displays use the ruled format from day one. Zones (I-G5) unaffected | flag | 🧾 sweep queued (build-queue X1 line) |
| the queue | **the block order RULED (owner, 2026-09-06):** Phase F gains item 7 — the board's public face (publish · review · marker · view-as-visitor; gates: 2b + modes) so F ends with the board publishable → compositions lead after F → the HOME LOOK-AGAIN last (P0 from-scratch of the display level; compositions on home; S3 folded in) | decide | ✅ folded |
| F · item 0 | ✅ **the safety sitting RAN (D-152)**: export real (media fetched) · backup position-named · **restore rehearsed — failed once, fixed, then exact** · 3 migrations verified live · RLS guard green+red · **suite 25/25 green today** (caught 1 stale fixture) | build | ✅ 2026-09-06 |
| F ruling | **the born-private rider RULED onto item 0** (owner 2026-09-06): bit default flips + legacy flip; migration proven on the full stack (2 public → 0 · newborn private · CHECK holds) | decide → build | ✅ proven, awaits the sitting |
| B ruling | **S8 ruled**: text-card height null = auto (default) · number = deliberately fixed (aesthetic option, design-pass era); false writes stop in card-split ③ | decide → build | 🧾 in 2b |
| L4 note | **R4 closed** — the handle's flat look stands as the owner's own function-first ask; the LOOK question moved to the design pass (after-F arc) | decide | ✅ 2026-09-05 |
