...
- **I-G5 — Every date is shown in the READER'S own zone (D-144, owner-ruled 2026-09-03).** A moment is stored once, in UTC (`timestamptz`); *which day it reads as* is decided by the device in front of whoever is looking — phone, laptop, another city. **The server may only GUESS the first paint** (`x-vercel-ip-timezone`, UTC locally) and the device corrects it on hydration, so **a wrong guess costs nothing**. A pinned zone is forbidden: it is silently wrong the moment the app is opened somewhere else. A stored preference, when one exists, is an **override** of the device — never a replacement for asking it. → `app` (single chokepoint: `lib/dates.ts`) **+ a boundary test that fails on any date formatting outside it** — the same shape as the no-database-outside-`lib/db` rule, which has held in 96 files out of 96.
