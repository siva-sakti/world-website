...
- **⚠ I-T4 IS UNDER QUESTION (opened 2026-08-28, D-122; not amended, not broken — flagged).** A proposed *archive* state ("put away, still findable") collides with this rule. An archived row is a **live** row, so under I-T4 it is **in the world** — therefore in all five world surfaces (the pull · find · home · boards · the graph). "Put away" as designed wanted it out of four of them while staying in find, which is a **fourth domain** by another name. Two ways out, both rulings the owner has yet to make: **(a)** `world` splits into *at hand* and *put away*, I-T4 becomes "four domains" and `lexicon.md`'s world entry is amended; or **(b)** archive is a *listing property only* — it removes a thing from the index surfaces (`/notes`, `/bits`) and changes nothing else, leaving I-T4 untouched. Until one is ruled, **no surface may filter on archive** beyond what already ships, or the fourth domain arrives by accident. Reasoning: `deliberations.md` (the archive round); re-entry: `parked.md` A24.
