...
**Verified facts:** only the **text** bit type has a body at all (drawing = `strokes` jsonb · image/audio/pdf = files · link = url), and that body is **HTML**; the composition's is **JSON**. **ONE piece of code IS the writing box** — the `TextBit` component (`src/app/board/[id]/text-bit.tsx`). It is not copied; the SAME code is mounted in three places today — **inside a board's text card** (`board/[id]/card.tsx`) · **on a bit's page** (`bit/[id]/text-workspace.tsx`) · **in quick-write** (`write/quick-write.tsx`) — and the composition page becomes the fourth. **Consequence:** teach that component checklists and checklists exist EVERYWHERE it is mounted, board text-cards included. There is no "only in compositions" without a deliberate choice. **The ruled resolution (§13): the CAPABILITY is shared; what is EXPOSED differs by place** (§13.4's per-frame toolkits) — so at build: add the capability once, choose the exposure per mount.
