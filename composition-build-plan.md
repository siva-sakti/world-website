...
**Accept ①a:** *(+ the four floor rules)* the ①a-suite green incl. old-app-unmodified assertions · the owner notices nothing changed.
**Accept ①b:** *(+ the four floor rules)* the ①b-suite green (kind gone AND both views return rows · counts match) · the S-C scenes seeded fresh in the new home and replayed · export completeness · the grep receipt clean · bits/boards exactly as they were minus `kind`.
...
**Accept:** *(+ the four floor rules; this stage introduces the composition page AND the list — the three states apply to both)* the eight musts minus drag · list sort/filter works · a save against a just-trashed composition prompts instead of landing silently.
...
**Accept:** *(+ the four floor rules)* the §24.6 scenes S-C1/S-C3 exact · a pasted image survives trash/restore · destroy sweeps bytes (proven) · promotion lands the file in bit ownership.
...
**Accept:** *(+ the four floor rules; focus handling across the frames, and the phone rule, land here)* the open-chain feel-test list (tech-spec §5) · evaporate's acceptance (create-empty-close ⇒ zero rows) · the floater's save status visible (§6.6 closed).
...
**Accept:** *(+ the four floor rules)* §26.3's table demonstrated row by row; positions survive a reload; restore brings a pinned window back.
...
1. ⭐ **§15 · Empty, loading, error — every screen.** Spans every stage and belonged to none. **Ruled here, and ENACTED as the first of the four floor rules above** (not merely recorded — every stage's Accept line now references it). No stage is done until its new screens have their three states. *(The owner: "if loading or [a fetch] fails, we need to write those things down.")* Stage ③ additionally owes the block's states (§34.5), which §15 never covered.
