...
*(The owed "is anything lost?" pass over `composition-surface-spec.md` and the two `convergent-surfaces-*` files, done while the adversary ran. Most of their content is carried or superseded — these four are NOT, and three are genuinely useful build knowledge.)*
1. ⭐⭐ **THE BLOCK IS MOSTLY ALREADY BUILT — it is "the peek, made permanent."** `BitRefView`'s peek **already fetches the target and renders its content** (thumbnails, the glimpse). So block mode ≈ a `display: chip|block` attribute on the existing `BitRef` node, reusing the peek's fetch-and-render. **The verified spec calls the block "ruled, unbuilt" and never says this** — it materially changes stage ③'s size estimate downward. *(Source: `composition-surface-spec.md` §4.4.)*
