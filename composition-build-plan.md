...
⭐ **THE BETTER ANSWER (revised 2026-09-04, after the owner asked what testing the editor even means): most of the RISK is not the typing — it is the LOGIC.** *"Which blocks does this heading own?"* is a pure question — give it a document, get an answer — and **written that way it is fully testable today, no new dependency.** The same for: what a fold contains · what the search-extractor pulls out · which occurrences the reconciler derives · the display-label fallback.
...
4. **The unpublish corner** — ⚠ **REOPENED BY THE OWNER 2026-09-06, awaiting her ruling** (`docs/accounts-session-notes.md` N12–N13): the sealed answer (unpublish the piece with her confirm) was a late one-line swap from an earlier ruling and never walked; three live options — **A** unpublish *(sealed; Claude's recommendation — the only one where nothing bad is visible to anyone, and the only fully reversible one; if its sting is 'quietly gone', the fix is a screen: a visible "unpublished when you made [bit] private — republish?" state on the piece's page)* · **B** remove the quote *(app edits her prose)* · **C** hole + loud repair flag *(a visitor may see the hole)*. **Build nothing here until she rules;** whichever wins, it lands at this one piece — the rest of Stage Ⓥ is unaffected, and the BOARD half is untouched (the boards' absence law is a different law).
