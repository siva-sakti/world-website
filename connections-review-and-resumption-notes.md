...
**What this is:** the complete, consolidated verifier feedback on `connections-build-plan.md` (the "pull a bit into a thought" + reference-graph feature), given across the owner–advisor review conversations of 2026-07-24 — preserved top-to-bottom because the owner **parked the feature the same day** (D-098). **Revision state at parking:** partially applied — the vision section now leads the plan (ask #1 ✓); the plan's own banner names the ONE open decision (Stage 5: tags/boards as graph layers or fully out — the same fork as blocking finding #2 below); the remaining asks and findings below are still to land. When the feature is picked back up, this doc + the plan are the complete starting state: nothing needs re-deriving. **Status of the model: §6 ("no stored bit↔bit fact") was NEVER actually reopened** — the plan *proposed* reopening it; no ruling was recorded, nothing was built, the schema is untouched. The owner's stated intent stands on record: *"I do want this for sure — that's philosophically; we'd have to change some things up."*
