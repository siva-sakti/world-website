...
> **📜 Older product-phase sketch (D-085), frozen pre-notes.** The **live plan / build queue is now `build-queue.md`** (Phases O/V/N). Two claims below are stale: "**no `kind`**" is false — `bit.kind` ('bit'|'note') is the load-bearing field of the whole bit/note distinction (D-119/D-121); and `agreements.md` "the ruled model" is historical now — the current model is **`model.md`**. Kept for the phase-shape background.
