# Agreements — the early model, in detail  📜 HISTORICAL

> **📜 HISTORICAL RECORD — do NOT build from this doc.** These are the **D-019→D-102 era rulings** (2026-07-19/20), frozen when the early conceptual model was CLOSED (D-070). **It predates the entire notes era (D-118–D-121)** — it never mentions notes, `bit.kind`, or the bit/note/board three-kinds model. The **current** model lives in **`model.md`** (words → `lexicon.md` · rulings → `PROGRESS.md` D-log). Kept only as the record of *how the early model was reasoned*; where it conflicts with `model.md`, `model.md` wins.

**What this was (as of 2026-07-20):** the complete record of everything agreed in the owner–Claude working conversation of 2026-07-19/20 — the model, ruled piece by piece. Open items are listed at the bottom. *(The old "supersedes/outranks every doc" claim no longer holds — superseded by the notes era; see the banner above.)*
...
| type | text · drawing · image · *(later: pdf · audio)* — the half the machine can tell on its own · *(**bookmark** retired as a type — D-102; a URL is a source on a note, §2a)* | system |
...
**Source made first-class; bookmark retired — ruled (2026-07-26, D-102; SUPERSEDES the D-100 source ruling it replaces).** *(Proven on a throwaway copy, then applied to cloud — migration `20260726000001_source_first_class.sql`.)* The pivot: **a URL is a *source*, not a saved page.** There is no "bookmark" object and no auto-bookmark ever — "save a page to revisit / give it extra attention" is a **deliberate act, a tag** (consistent with the rejected favorites/star, §2e), never a default.

**The link bit — ruled (2026-09-01, D-129; PARTIALLY SUPERSEDES D-102).** *(Proven on a throwaway, then applied to cloud — migration `20260901000001_link_type.sql`; plan + full trace `link-bit-plan.md`.)* **A link can be a bit** — a first-class **`link`** type beside text/drawing/image/audio/pdf: the thing pointed at (an album, a video, an article) is *material*, placed and arranged like any bit, shown as a card with its read-once **captured title** + a **stored copy** of the page's card image (`thumb_path`, no `storage_path`). **D-102's source model stands untouched** — the capture rule is *"what you give is what it becomes"*: a **bare URL alone → a link bit**; **words + a URL → a note carrying it as a source**. The hoarding worry stays answered structurally: a link bit is always a **deliberate paste**, never an auto-save; "save to revisit" remains a tag. A link bit takes the intake's sticky source (the chip you set is always honored — owner-ruled); `asQuote` shapes notes only; duplicates are allowed (bits are material, not a registry); the pre-D-102 bookmarks-turned-notes **stay notes**. Display is **the card, not the embed** (players parked → A25). Degrade ladder: rich card → title card → URL card — **capture never fails**. The face rule revives (`content` → captured title → URL); search stays wider (title + URL indexed, both layers).
- **Source is a first-class vocabulary citizen** (the fourth vocabulary word — §7). A real named thing, **tag-shaped**: its own `source` table — a **name** (never blank) + an **optional `url`** (a web page has both; a book has a name, no link) — with **pick-or-create** autosuggest, **rename-once** (id-referenced, so a rename touches one row and every note re-labels — P9), and a **sources-list** (your reading list). **Universal** — any bit, any type, may carry a source. **One source per bit** (a bit has *many* tags but at most *one* "where it came from"); blank = self-made. Near-duplicate names refused case-insensitively at birth, like tags.
- **A record, read once, machine-never-re-reads.** A source is created at capture (or picked from the list) from the page's title, and the machine never re-fetches it — a dead or edited page can't rewrite what you filed (the storage test, principle 2). Only your deliberate rename changes it. If a title-fetch failed, the source is **named after its link** (name = the title if we have one, else the URL) — nothing is ever blank.
- **The old D-100 fields moved in; every bookmark became a note.** D-100's two frozen text fields (`source_url`/`source_title`) are migrated into real `source` records and dropped; every `bookmark` bit is rewritten into an ordinary **text note whose body is a clickable link**, carrying a source, and the `bookmark` type is retired (`bit_type_allowed` 4→3; the substance branch dropped, reverting D-100's preview relax). *(The now-retired D-100 lines — "a bookmark's source is itself," "a bookmark may carry a preview" — no longer hold: there are no bookmarks.)* → invariants **I-Src1–I-Src5** (retiring I-S3 · I-S4; I-R3 moot).
...
|---|---|
| **A. Truth — database** | the **nine record kinds, three families** (ruled 2026-07-20; **source** added 2026-07-26, D-102): **things** — bit · board · **acts** — tag application · placement · **connector** (§6a) · **vocabulary** — tag word · **category** (§3b) · **subtype word** (§5c) · **source** (§2a, D-102) — **plus one *derived index*, `reference`** (§6 amendment — gather's directed bit→bit ties; stored here in layer A but **layer-E-natured**, rebuildable from bodies, and in the export for completeness — the first record of its nature) — plus their small values: subtype, **`visibility` (on both bit and board — §2a)**, display size, **placement arrival/departure times (travel history, §2c)**, **a bookmark's captured title (§2b — now a dead-but-present column, D-102)**, **a bit's `source_id`** (its one optional source — §2a, D-102)… |
