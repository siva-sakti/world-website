...
| A12 | ✅ **RE-ENTERED + BUILT for the jot box (D-133)** — the `/bits` capture box mirrors to device-local storage and restores into the box, never to the DB (I-D3 held). *Still parked for the other editors* (the note workspace, `/write`) — those have save-guards but no crash-guard; additive anytime, same shape. | additive anytime; must restore *into the editor* only — never auto-replay to the DB (I-D3) | §2h · D-133 |
...
| A15b | ↩ **RE-ENTERED 2026-09-03 (the reverse direction): gathering a BOARD into a composition** — owner: "maybe they want to gather a whole board and reference it — that should be OK." The tie's target set becomes bit \| board \| composition (exactly-one CHECK, the A15 shape exactly as predicted). Ruled in `docs/composition-spec.md` §30; builds with the composition storage work | composition-spec §30 |
...
| A20 | **The multi-board call-in door** — ✅ **RE-ENTERED IN FACT (N2, 2026-08-26; drift caught + corrected 2026-09-01):** the board drawer's "other boards"/"anywhere" scopes now surface *already-placed* bits and a click places them on this board (`drawer.tsx` → `bringIn` → `callInBit`) — the D-036 differentiator lives. Also the bit page's "place on a board…" (otherBoards). What stays parked here: the *loose-page* "all" tab deliberately keeps select/send loose-only (the guard in notes-browser) | the remaining half re-enters only if the owner wants bulk multi-board sends from /bits | call-in plan §12 · D-104 · N2 |
...
| A24 | ✅ **SHIPPED — drift corrected 2026-09-02** (this row described the sent-back FIRST attempt and was never updated). Archive is built, migrated and live: `20260830000001_resting_state.sql` + `20260830000002_archive.sql` (D-127), the `/archive` room, ArchiveButton on home rows + the note page, and `20260902000002_archived_not_alive.sql` restoring the starred-vs-archived CHECK the first attempt lost. **The original three questions were answered by the resting-state design**; what remains parked is only the WORD ("archive" is software-speak; "put away" was rejected) and container-cascade for boards. *(History: the first attempt — `20260828000001_archive.sql`, throwaway-proven 8/8, never applied to any cloud, UI reverted — is D-128.)* | **Re-enter when the owner answers the three questions the audits isolated:** (1) **which surfaces** does a put-away thing leave? `world` has five (the pull · find · home · boards · the graph) — this is the ⚠ I-T4 question, and it must be settled first because it is the root cause, not a detail. (2) **What happens to a container's contents** when the container is put away? Following the trash precedent floods the loose pile; not following it strands the contents (see the deliberation). (3) **Is it reversible** — does the star come back? Trash→restore keeps it; archive→unarchive currently destroys it. *Then* the word gets ruled ("archive" is software-speak; "put away" was Claude's and was rejected by the owner), and only then the code | deliberations (the archive round) · invariants ⚠ I-T4 · `verification/archive-proofs.sql` · D-128 |
