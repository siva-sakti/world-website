...
Dead exports (`isBusy`/`version`; `display_size` stored, constrained, copied, never read) · stale comments asserting false facts (`DEFAULT_W = 240` "matches the render-layer default" — it is 400; `card.tsx:327` claims react-rnd suppresses click after a drag — **verified false, 0 occurrences**) · an unused `placementId` parameter on two remove doors · `looseRefresh` bumped on a path where nothing became loose · one unreachable branch · stale line references in `old/frame-plan.md`.
