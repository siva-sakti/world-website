...
- **`research-notion-data-model.md`** *(2026-09-03)* — **Notion's data model, every line quoted + sourced or marked NOT FOUND.** A reference point the owner asked for. Written under a hard citation rule after Claude asserted Notion's architecture unsourced (2026-09-02). **Establishes:** everything in Notion is a block *(Notion's own blog)* · the block record's fields · six parent shapes · **no documented ordering field** · the sharding numbers. **Does NOT establish:** why Notion chose it, what users value, or how blocks are really stored. Nothing in the live decision depends on it.
- **`composition-enactment-procedure.md`** *(repo root, 2026-09-04)* — ⭐ **STAGE ⓪'s OUTPUT: the complete procedure for the schema change**, written before it is needed — the enactment extras · the ordered operations (incl. the other window's two migrations first) · the app sweep and its grep receipt · the rehearsal · **the owner's checkpoint script** · **the back-out procedure**. Read in full before stage ①.
- **`how-each-piece-gets-built.md`** *(2026-09-04)* — ⭐ **THE PER-FEATURE RUNBOOK**: find its spec home → say "done" in her words → name what it could break → adversary on the plan → build small → **prove with raw output** → clear the floor (three states · no hover-only · keyboard+phone · green) → **she uses it for real** → record same-session. Point at it per feature; the build plan says what and when, this says how.
- **`vocabulary-workshop.md`** *(2026-09-04)* — 🟡 **THE APP-WIDE VOCABULARY WORKSTREAM** (the owner's, worked in her own window): the full word inventory · what's settled and unreopenable · the open questions with HER reactions recorded (bit×piece as one PAIR decision · gather's two jobs · the two-surfaces analysis · "block" as a confirmed defect). `lexicon.md` stays the authority; words graduate INTO it.
- **`accounts-session-prep.md`** *(2026-09-05)* — the next thinking thread's prep: GOALS first (her recorded wants + the never-list to re-choose) · what already exists (per-row ownership = multi-home one config away) · the touched surfaces · six scenes · the accounts-then-social order.
- **`visibility-model.md`** *(2026-09-05)* — ✅ **THE AUTHORITY for everything public/private, SEALED by the owner**: the switches · the three lived stages · the two medium-true laws (boards absorb absence; a private bit can never be part of a public composition) · the states and acts tables · the invariants · the corners · the companions · the technical delta. Full reasoning: `privacy-session-notes.md` (N1–N47). **Supersedes every earlier privacy statement — the sweep table inside names each.**
- **`privacy-simplification-review.md`** *(2026-09-05)* — the fresh-eyes flexibility-without-complexity review: the pay-as-you-go finding, the five time-stumbles incl. the path-dependent two-key hole, three options stressed, the recommendation (current minus dial minus copy-offer) and THE one deciding question for the owner.
- **`privacy-session-prep.md`** *(2026-09-04)* — the privacy sitting's prep: what's already built and proven (the guest door, the AND-law), the founding public-vs-built-private tension it must resolve, the composition run's parked items, seven scenes to walk. Gates accounts/social/premium/share-sheet.
- **`naming-session-prep.md`** *(2026-09-03)* — the naming sitting's question sheet (composition-confirm · gather · direction words · the hover layer's name · small stamps); constraints and the owner's own seed-words only, no generated names — she writes the voice.
- **`composition-build-plan.md`** *(repo root, 2026-09-03)* — ⭐ **THE COMPOSITION'S BUILD PLAN: six gated stages** (① the split+migration, one enactment, owner-gated → ② editor core → ③ bring-in → ④ material doors → ⑤ postures → ⑥ hover layer), each citing the VERIFIED spec + naming its acceptance. Written after D-145/D-146; the builder follows it and improvises nothing.
- **`research-tiptap-persistence.md`** *(2026-09-03)* — **the evidence behind the composition's storage-format choice**: tiptap's own docs recommend persisting **JSON**, and HTML parsing is schema-strict (unknown tags are silently dropped). Also records where Claude's *original* argument for JSON turned out to be unsupported. Feeds `docs/composition-spec.md` §21.5.
