...
| **frame** | ⛔ **not used in this spec.** The word belongs to the board track — *(updated 2026-09-05)* a **KIND of board**: fixed-size, chosen at creation (`frame-spec.md`); the earlier "alignment feature" description is superseded. |
...
| **direction** | **Boards hold; compositions are held.** A composition never CONTAINS a board — its contents never pour into writing. ✅ **AMENDED (owner, 2026-09-03, resolving the §30 collision): a composition MAY REFERENCE a board — as a DOORWAY ONLY** (its title as a chip; its card as a block; never its cards exploded). *"We're gonna change that — I think compositions can reference boards"; Claude concurred: the law's purpose (no containment) survives; mention ≠ containment was already its own clause. Supersedes register F3/C2 and §9.2.4's boards-never; the picker gains boards; a board-side "referenced in…" surface is deferred.* **Mention is not containment** — a plain hyperlink to a board remains possible: it is an **ordinary editor link** (the link mark that already exists), **not a feature to build**, and it creates **no stored tie, no backlink, no graph line.** | owner |
...
4. ✅ **THE VISIBILITY MODEL ARRIVED (2026-09-05): `docs/visibility-model.md` is the authority** — everything born private; boards absorb absence; **a private bit can never be incorporated into a public composition** (the barrier · the hover view · the resolve-at-publish · the unpublish corner). *(This item previously read "deferred to the privacy session" — the session happened.)* 📜 superseded text: *(owner, 2026-09-02: "we still have a lot to decide in terms of private and public — I think we haven't even touched that yet. Right now everything is public; later we're gonna have toggles and we have to think through that logic.")* **What stands:** controls should exist, and a composition should be born private. **What is NOT decided:** inheritance (what an unmarked bit does on any board), the toggle logic, and everything about publishing. **Do not build visibility from this section** — it is the privacy session's subject.
...
- **Why the owner's own evidence settles it:** Notion's **insert menu offers "toggle heading 1/2/3"** as their own entries — they exist *because* a plain heading owns nothing. Section-dragging is available there by explicitly nesting content inside a container, visibly. *(Source: the owner reading the palette aloud with Notion open, 2026-09-06 — direct observation, this project's strongest evidence class.)*
  - ⚠ **One source tension, recorded rather than smoothed over:** this repo's own `research-notion-data-model.md` §8 enumerates Notion's **API** block types (`heading_1 … heading_4 · toggle · …`) with **no `toggle_heading` value**. That file already warns its own enum is approximate. The likely reconciliation — **labelled inference, unverified** — is that the API models a toggle-heading as a heading carrying a flag rather than as its own type, i.e. a UI-vs-API difference, not a contradiction. **Nothing in D-153 rests on this:** the ruling rests on the flat-drag behaviour the owner observed directly, and would stand even if Notion had no toggle-heading at all.
...
- ⚠ **And note what does NOT need blocks:** folding a section · dragging a block · the table of contents. All are **editor behaviour**, not filing. *(D-153 trimmed this line's middle item — "dragging a heading with its section" is no longer a behaviour that exists.)*

**What the feature list actually demands** — only three lines have teeth:

| feature | demand on storage |
|---|---|
| rich text · drag a block · fold an explicit container · table of contents · images · undo | **nothing** *(D-153 split the old "fold/drag a heading": dragging a heading is one block like any other; folding belongs to an explicit container)* |
...
- **Write:** type/Enter/Shift+Enter 🔨 · bold·italic·quote·lists 🔨 · headings ✅(🔨 unstyled) · checklist ✅ planned · a table you make = writing ✅ · `/` menu (typed `/` only, never re-fires on paste/undo) ✅ · collapsible sections ✅ wanted (⚪ **now narrowed by D-153**: heading-is-the-unit is DEAD with the span; the choice is an explicit toggle block vs headings gaining an explicit fold)
- **Move writing:** cut/copy/paste the main path ✅ · drag block up/down ✅ · **a drag moves exactly the dragged block — no inferred spans** ✅ *(D-153; the former "heading carries its section" is superseded — see §20.5)* · folded+backspace → unfold first ✅ *(applies to explicit containers)* · undo ~15 ✅
...
4. ✅ **RULED (owner, 2026-09-03): the COLUMNS BLOCK is IN; wrap is OUT** ("I think columns block should be fine" — her own sketch: image in one column, text beside, resizable divider; text does NOT continue beneath). **Supersedes** §13.1's columns-⛔ · the §25 no-columns limit — her word covers those. ✅ **The wrap-out half CONFIRMED by her word 2026-09-03 (§33.1)** — the P1-D2 flag is closed. **STANDS untouched:** float-never (E4's other half) · the boards test itself (callouts stay dead). The §30c gallery lean folds in (a gallery ≈ image columns).
5. ✅ **The export renderer APPROVED** (owner, 2026-09-03: "I'm fine with it as long as it's thought out and built well") — `@tiptap/static-renderer`, installed at enactment, export-route only. ✅ *(Package confirmed by her word, 2026-09-03 — §33.2; the P1 flag is closed.)*
...
**Columns** — §13.9 below is the 🔵 builder-default; the owner's refinement pass owns the feel. **Toggle mechanism** — ⚪ **re-framed by D-153:** heading-as-the-collapsible-unit is **dead** (it rode the span). The live choice is **an explicit toggle block** vs **headings gaining an explicit fold of their own** — either way the container is explicit and its contents really nested, never inferred. The builder implements §13.7 as written; the ⚪ narrows to which of those two shapes. *(Notion's own answer, from the owner's palette read-out 2026-09-06: a separate "toggle heading" block type — offered, not ruled.)*
...
1. ✅ **CONFIRMED (owner, 2026-09-03): wrap is gone for good.** The §13.2.2/E4 supersession now rests on her word, not a framing.
...
4. 📜 ~~a public piece can have holes (private windowed bits render absent)~~ **SUPERSEDED (2026-09-05, `visibility-model.md`): the mixed state is PREVENTED, never rendered** — a private bit can never be incorporated into a public composition; a public piece never shows differently than authored. The corner that remains: a later privacy flip un-publishes the quoting piece, with her confirm.
