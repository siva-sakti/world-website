...
- ⭐⭐ **THE REAL ANSWER (owner asked directly: "if Notion does that, can/should we do this too?") — we already have the good half, in a better place.** Notion promoted the *paragraph* into an addressable reusable unit **because Notion has no concept of material** — it has only pages made of paragraphs, so the paragraph was the only candidate. **This project has bits.** The addressable reusable unit already exists as its own kind of thing, and a better one: a bit can be an image, a drawing, a voice memo, a whole PDF, and it can sit on many boards at once, alive on all of them.
