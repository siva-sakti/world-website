...
export function BoardSurface({
  ...
  // Dragging a card: the magenta guides + moving a whole selection together (S1 — lifted
  // out whole into use-card-drag; the board keeps the wiring, not the machinery).
  const {
    vGuideRef, hGuideRef, snapDrop, onCardDragStart, onCardDragMove, onCardDragEnd,
    cancelDrag, dragStart, dragBefore,
  } = useCardDrag({
    cards, setCards, selectedIds, boardRef, camRef, screenToWorld, sizeOf,
    patchCard, recordGroupMove: (moves) => arrange.recordGroupMove(moves),
  });
