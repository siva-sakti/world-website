...
| **an opening** *(D-134, stored — the ninth-and-a-bit table `opening`)* | the owner's act of **opening** a board or a note. One row per thing per owner, its `opened_at` moved forward on each open — so it dedupes structurally and never accumulates. Feeds **"where you were"** on home. **Deliberately NOT the word "visit":** `init.sql:357` already promises "one-row-per-visit" to the visit-by-visit **travel** timeline (parked A7 — where a *bit* has been). An opening is where the **owner** has been; travel is where a **bit** has been. Private always — no guest policy, ever. |
...
| **where you were** *(D-134)* | the last **~5 surfaces you opened**, newest first, on home. Computed each load from the newest `opening` rows resolved against the surfaces home already holds — so a renamed board is never stale, and a trashed or archived one falls out **with no filtering of its own** (it simply isn't in the live list). Distinct from **"recently modified"**, which only knows what you *changed*. |
...
- **face** — what a bit **shows** (its display headline); **computed, never stored**: your `content` if any, else the mechanical fallback per type — text: the body's plain words · **link: the captured title, else the URL** *(the D-102-retired bookmark rule, revived for the `link` type — D-129)* · pdf/audio: the file's metadata title · drawing/image: nothing (the visual self). **The face's first words are the bit's de-facto title**; on a text bit an **optional written title** (content) stands in for the first line (D-087). **Search is separate and wider** (D-088): it indexes *all* a bit's words (content + body + captured-title + URL), never just the face — so a titled note stays findable by its body.
- **captured title** — a **link** bit's page-title, read **once at save** and stored as machine truth beside the URL, immutable thereafter (the live web can't be re-read — §2b). To rename a link, you write `content` — your act, your column. *(Retired with bookmark at D-102; **revived for the `link` type at D-129** — the read-once principle also covers the link's stored card image, `thumb_path`.)*
...
| **bookmark** (the bit type) | retired as a concept (D-102) — and **stays retired as a WORD** after D-129 revived the saved-page *object* under the name **link** (a bare URL pasted deliberately becomes a `link` bit; words + a URL are still a note-with-source). "Save to revisit" = a deliberate **tag**. |

## The "link" collision — CLOSED, then retired (D-074 → D-102)

**History.** "link" was retired as a *relationship* (§6) but briefly still named a *bit type* — ruled **`bookmark`** (D-074) to close the collision. **D-102 then retired `bookmark` itself** — a URL is a **source** on a note, not a saved page. **D-129 (2026-09-01) revived "link" with exactly ONE live meaning: the `link` bit type** (the saved-page object as a card — read-once title + stored card image). The *relationship* sense stays dead; "bookmark" stays dead; a hyperlink inside prose is still just a hyperlink.
