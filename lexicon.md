...
**The one distinction everything hangs on: STORED vs COMPUTED.** Eight kinds of thing — in three families, **things · acts · vocabulary** — are written to the database. Everything else you see — the pull, find, the graph, a bit's page, a bit's *face* — is *computed on the fly* from those eight. Never confuse a stored thing with a computed view of it.

---

## The eight stored things (the whole database) — three families
...
*(A ninth table exists but is empty and dormant — the deferred pairwise "link." It is not part of v1.)*
...
**make / jot** a bit · **tag** it · **place** it (→ a placement) · **un-place** it (take a card off a board — kept in travel history) · **pull** a tag (→ the pull) · **connect** two cards (→ a connector) · **call in** an existing bit onto a board · **trash / restore** a bit **or board** (freeze / unfreeze — §2g).
...
- **tables** — `bit` · `board` · `tag_application` · `placement` · `connector` · `tag` · `category` · `subtype_word` · `dormant` (the nameless ninth — "dormant" describes its state, deliberately not a product word)
...
- **views** (the named surfaces, computed) — `the_pull` · `the_ledger` · `home` · `trash_listing` · `bit_travel` · `board_cards` · `board_connectors` · `tag_counts` · `subtype_word_counts`.
