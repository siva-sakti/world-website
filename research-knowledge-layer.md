...
This app already has a schema and vocabulary in place (`SPEC.md`, `map.md`): `bits` (fragments) live independently of `canvases` (boards) and can be placed on zero/one/many of them (`placements`); `tags` are flat, id-referenced, and joined via `bit_tags`/`canvas_tags`; `links` is a bare `from_bit_id → to_bit_id` table; topic-pages and a scoped `/graph` are already planned but marked 🔴/not-built. This research is aimed squarely at unblocking those two rows.
...
- Both gestures write to the *same* `links` row shape, so the picker can ship alone (unblocks the 🔴 "Linking" item in `map.md` immediately) and the canvas-drag gesture can follow once canvas mode exists.
...
2. **Link picker (non-canvas)** — a "Link" affordance on `/b/[id]` and inside the Tiptap editor that opens a search-and-pick modal over existing bits (Heptabase/Notion `@`-pattern, tap-triggered). Writes a row to the existing `links` table. Unblocks the 🔴 "Linking" item in `map.md` without touching canvas code at all.
