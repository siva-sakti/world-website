...
- **Transport rule (app-wide):** media uploads go **client → Supabase Storage directly** (authenticated + storage RLS), **never through a Vercel function** — serverless bodies cap ~4.5 MB; iPhone photos are 3–8 MB. **One ruled exception (D-129):** a **link bit's** card image is fetched *by the server* (the og:image/oEmbed read) and uploaded server-side with the same user-scoped client — the rule's reason (the inbound body cap) doesn't apply, since the bytes originate on the server (capped at 4 MB by the fetcher).
